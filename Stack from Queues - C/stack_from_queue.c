...
void listQueueInit(struct Queue* queue)
{
    ...
    assert(sentinel != 0);
    sentinel->next = 0;
    sentinel->value = -7734; //this line is unnecessary, but may be useful as a flag for error testing
...
{
	/* FIXME: You will write this function */
    struct Link* newLink = (struct Link*) malloc(sizeof(queue)); //Declare new link and allocate memory
    assert(newLink != 0); //ensure malloc worked properly and that newLink does not equal null (0)
    newLink->value = value; //Assign TYPE value to newlink->value
    printf("ADDING %d to BACK of QUEUE!\n", value);
    if (listQueueIsEmpty(queue) == 1)
    { //Condition if list IS EMPTY
        queue->head->next = newLink; //Sentinel (after head) points to newLink
        queue->tail = newLink; //lastLink points to newLink, too, since there is only one element
        newLink->next = 0; //set newLink to null (0)
    }
    else
    {
        newLink->next = queue->tail->next; //set newLink's next value to the current last value's next pointer (should be null, or 0)
        queue->tail->next = newLink; //set current last link's next pointer to newLink
        queue->tail = newLink; //Finally, make lastLink point to newLink
    }
}
...
struct Stack* listStackFromQueuesCreate()
{
    ...
    stackPtr->q1 = listQueueCreate();
    stackPtr->q2 = listQueueCreate();
    return stackPtr;
};
...
{
	/* FIXME: You will write this function */
    struct Queue* tempQueue = listQueueCreate();
    tempQueue = stack->q1;
    stack->q1 = stack->q2;
    stack->q2 = tempQueue;
}
...
void listStackPush(struct Stack* stack, TYPE value)
{
    ...
    if (listQueueIsEmpty(stack->q1) == 0)
    {
        do
        {
            TYPE temp = 7734;
        ...
        } while(listQueueIsEmpty(stack->q1) != 1);
    }
...
TYPE listStackPop(struct Stack* stack)
{
	/* FIXME: You will write this function */
    assert(stack != 0);
    TYPE returnValue = 7734;
    if (listStackIsEmpty(stack) == 0)
    {
        returnValue = listQueueRemoveFront(stack->q1);
    }
    return returnValue;
}
...
TYPE listStackTop(struct Stack* stack)
{
    ...
    if (listStackIsEmpty(stack) == 0)
    {
        returnValue = listQueueFront(stack->q1);;
    }
